...
## Update log
### 2025-11-22 Version 2.1.4:
- 1. Improved the robustness of Vertical LAH indicators (HIP, LAR) calculation by adding physical constraints and optimizing the fitting process.
- 2. Fixed the surface area calculation in 3D Cluster analysis to ensure accuracy on triangulated meshes.
- 3. Corrected the class names and method calls in the documentation.
...
siteLA.computeBatch(CPU_count=4)
```

### 3D Light Availability Heterogeneity (LAH) analysis
The LAH is calculated by the `LAH_analysis` class. The `field` is the LA field to be calculated.
...
### Visualization
The `Visualization` module is used to visualize the data in the SFL and the results of LAH analysis. 
